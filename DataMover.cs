...
namespace DataMover
{
	public static partial class DataMover
	{
		...
		public static List<Constraint> DatabaseConstraints;
		...
		static int Main(string[] args)
		{
			...
			{
				TraceLog.WriteConsole($"Using default command file [{CommandFileName}]");
			}
			else
			{
			...
			try
			{
				...
				TraceLog.WriteConsole(timer);
				TraceLog.WriteConsole("---------------------------------------------------------------------------");
			}
			catch (Exception exc)
			{
