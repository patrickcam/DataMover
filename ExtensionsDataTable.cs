...
namespace DataMover
{
	public static class ExtensionsDataTable
	{
		...
		{
			TraceLog.WriteConsole($"{destTableName} write count: {dataTable.Rows.Count}");
		}

		public static async Task<int> WriteToServer(this DataTable dataTable, SqlTransaction sqlTxn,
			List<SqlBulkCopyColumnMapping> columnMappings, SqlBulkCopyOptions bulkOptions = DataMover.DefaultSqlBulkCopyOptions)
		{
			...
			})
			{
				...
				await useCpy.WriteToServerAsync(dataTable);
			}
